...
public class BookingService : IBookingService
{
    ...
    public Booking Book(int userId, string categoryName, DateTime startDate, DateTime endDate, Currency currency)//основной метод пон€ти€ брони
    {
    ...
    public void CancelBooking(Guid bookingId) //метод закрыти€ брони
    {
    ...
    private static decimal CalculateDiscount(int userId) //калькул€ци€ скидки
    {
        return 0.1m;
    }

    public Booking? FindBookingById(Guid bookingId) //метод поиска брони по id
    {
        return _bookings.FirstOrDefault(b => b.Id == bookingId);
    }

    public IEnumerable<Booking> SearchBookings(DateTime startDate, DateTime endDate, string categoryName) //поиск брони
    {
    ...
    public decimal CalculateCancellationPenaltyAmount(Booking booking) //метод расчЄта штрафа за отмену брони
    {
        if (booking.StartDate <= DateTime.Now)
        {
            throw new ArgumentException("Start date cannot be earlier than now date");
    ...
    private static decimal GetCurrencyRate(Currency currency) //получение рейтинг
    {
    ...
    private static decimal CalculateBookingCost(decimal baseRate, int days, int userId, decimal currencyRate) //метод расчЄта стоимости брони
    {
